...
// Use this function to generate detailed CPU trace
void MicroOpPerformanceModel::CPUTraceGen(DynamicInstruction *dynins)
{
   ...
   {
      size_t m = 0;
      for(std::vector<const MicroOp*>::const_iterator it =
          dynins->instruction->getMicroOps()->begin();
          it != dynins->instruction->getMicroOps()->end();
          it++)
      {
